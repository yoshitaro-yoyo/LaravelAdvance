 ...
 //③−2 マイグレーションテーブル作成 一度実行された実行済みとしてマイグレーションファイルに格納される。再度実行しても何も起きない。
    ...
    public function up()
    {
        ...
        Schema::create('movies', function (Blueprint $table) {
            ...
            //が削除された場合は、この動画情報も一緒に削除される」この外部キー制約によって、テーブル同士の相互性を高める。例えば、１つの動画が存在しないユーザの所有物として登録された場合、
            //動画自体はテーブル上に保存されるのですが、存在しないユーザに属するということはユーザに所属していない扱いになってしまいます。その場合、動画は存在するにも関わらず表示されない可能性等があり、
            //その動画はデータとして不十分なものとなる。つまり、外部キー制約を設けることによって、そのような中途��端なデータを生み出さないように対策を打っている。
